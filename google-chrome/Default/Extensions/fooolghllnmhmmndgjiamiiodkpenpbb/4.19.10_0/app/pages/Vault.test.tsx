...
async function renderList(route = ROUTES.VAULT) {
  ...
  const result = render(wrapWithProviders(<Vault />, { history }));
  await result.findByRole('list');
  return result;
}

let listeners: Array<Record<string, any>> = [];
const triggerNotification = (data: any) => listeners.forEach(({ cb }) => cb(data));
(createListener as jest.Mock).mockImplementation((name, cb, type) => {
  listeners.push({ name, cb, type });
  return jest.fn();
});
...
describe('Vault', () => {
  ...
  it('should sort list by name A-Z', async () => {
    StorageApi.get = jest.fn().mockResolvedValue({
      [Storage.VaultSorting]: { type: SortingType.Alpha, sort: SortingDirection.Asc },
    });
    const { queryAllByRole } = await renderList();
  ...
  it('should sort list by name Z-A', async () => {
    StorageApi.get = jest
      .fn()
      .mockResolvedValue({ [Storage.VaultSorting]: { type: 'alpha', sort: 'desc' } });
    const { queryAllByRole } = await renderList();
