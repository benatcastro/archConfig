...
import { isPassword } from '@common/utils/itemTypeGuards';
import { ExtensionContext } from '@extension/app/context/ExtensionContext';
import { IItem } from '@common/interfaces';
import Item from '../Item/Item';

const SuggestedItems: FC<Record<'items', Array<IItem>>> = ({ items }) => {
  const { domain } = useContext(ExtensionContext);

  const [suggestedItems, setSuggested] = useState([]);
  useEffect(() => {
    if (!domain) return;
    const filtered = items.filter(item => isPassword(item) && item?.url?.includes(domain));
