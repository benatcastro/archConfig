...
import { Text, Button, Checkbox, Loader } from '@nord/ui';
...
const DeleteFolderModal: FC<IDeleteFolderModalProps> = ({ button: OpenButton, folderId }) => {
  ...
  const handleDeleteFolder = useCallback(async () => {
    ...
    await api.item.delete([{ uuid: folderId }], deleteItems);
  }, [deleteItems, folderId, folderItems]);
  ...
  const handleDeleteButtonClick = () => folderItems.length > 0 ? setOpen(true) : deleteFolder();

  return (
    <>
      <OpenButton onClick={handleDeleteButtonClick} />
      {isOpen && (
        <Modal
          isOpen
        ...
        >
          <div className="relative">
            <div className="flex justify-between items-center mb-5">
              ...
              <ActionButton icon={closeIcon} onClick={() => setOpen(false)} />
            </div>
            ...
            {folderItems.length > 0 ? (
              <>
                ...
                <Checkbox
                  ...
                  onChange={() => setDeleteItems(!deleteItems)}
                  label={
              ...
              <Text variant="body2" className="color-primary">
                <FormattedMessage id="aboutToDeleteFolderQuestion" />
              </Text>
            )}
